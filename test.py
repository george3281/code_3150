...
# Resolve path to program.out: same directory as this test file (code_3150)
_PROJECT_ROOT = os.path.dirname(os.path.abspath(__file__))
_PROGRAM_PATH = os.path.join(_PROJECT_ROOT, 'program.out')


def run_program(input_data):
    """Run the homework executable with given input and return the result."""
    if not os.path.isfile(_PROGRAM_PATH):
        raise FileNotFoundError(
            f"Executable not found: {_PROGRAM_PATH}\n"
            "Build your C++ program first, e.g.:\n"
            f"  g++ -o program.out src/main.cpp src/*.cpp -I include"
        )
